  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard label="Imóveis" value={propertyCount ?? 0} subtitle="publicados" color="#5a5aff" />
        <StatCard label="Leads" value={leadCount ?? 0} subtitle={`${hotLeadCount ?? 0} quentes 🔥`} color="#4aff4a" />
      ...
      <div>
        <h3 className="text-zinc-500 text-xs uppercase tracking-wider mb-3">Últimos imóveis</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {recentProperties?.map((p) => <PropertyCard key={p.id} property={p} />) ?? (
            <p className="text-zinc-600 col-span-full text-center py-8">Nenhum imóvel ainda. Crie o primeiro!</p>
          )}
        </div>
      </div>
    </div>
